        ...
        case
            when (enc.observations ->> '3981ddb0-30a3-43d2-9564-16ae9cc0e25e')::numeric < 37        -- weight of women  in encounter
                then enc.observations ||
                     ('{"9a7f284b-251d-459b-97d9-929ed280b3d3":' || (case when enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' is null then '[]' else enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' end || '["ee723b88-6094-40b2-8db2-16368ae66f4c"]'::jsonb)::text || '}')::jsonb
           end,
     last_modified_by_id   = (select id from users where username = 'beulah@apfodishauat'),
     manual_update_history = 'added weight of women in high risk'
where
      enc.organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1)
...
set observations  =
        case
            when (enl.observations ->> 'a0059121-6bd2-414b-bcdc-a64bf27bc364')::numeric < 148        -- height of women in enrolment
                then enc.observations ||
                     ('{"9a7f284b-251d-459b-97d9-929ed280b3d3":' || (case when enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' is null then '[]' else enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' end || '["3f36ab23-e475-48d9-9bf9-556cab575303"]'::jsonb)::text || '}')::jsonb
           end,
     last_modified_by_id   = (select id from users where username = 'beulah@apfodishauat'),
     manual_update_history = 'added height of women in high risk'
...
set observations  =
        case
            when enl.observations ->> '72f8785c-f064-4549-ab45-47defa40f5fb' = '8ebbf088-f292-483e-9084-7de919ce67b7'     -- Red Flag (refer MCP card) in enrolment
                then enc.observations ||
                     ('{"9a7f284b-251d-459b-97d9-929ed280b3d3":' || (case when enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' is null then '[]' else enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' end || '["b9b4ac92-2aea-43d0-9d1c-bc7fd5026c44"]'::jsonb)::text || '}')::jsonb
           end,
     last_modified_by_id   = (select id from users where username = 'beulah@apfodishauat'),
     manual_update_history = 'added red flag mcp in high risk'
from program_enrolment enl
where
      enc.organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1)
  and encounter_type_id = (select id from public.encounter_type et where name = 'ANC' and organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1) )
...
set observations  =
        case
           when (enl.observations ->> 'c49442e4-539f-4d73-9590-0deb8c8dd3b9')::numeric >= 4  -- Gravida (the number of pregnancies the woman has had) in enrolment
                then enc.observations ||
                     ('{"9a7f284b-251d-459b-97d9-929ed280b3d3":' || (case when enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' is null then '[]' else enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' end || '["5a33dde5-f5f4-49ff-acc1-3d7ede26dc52"]'::jsonb)::text || '}')::jsonb
           end,
     last_modified_by_id   = (select id from users where username = 'beulah@apfodishauat'),
     manual_update_history = 'added gravida in high risk'
from program_enrolment enl
where
      enc.organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1)
  and encounter_type_id = (select id from public.encounter_type et where name = 'ANC' and organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1) )
...
set observations  =
        case
          when ind.age >= 35 -- age grater than 35
                then enc.observations ||
                     ('{"9a7f284b-251d-459b-97d9-929ed280b3d3":' || (case when enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' is null then '[]' else enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' end || '["ad71b3c0-4d95-4f2b-892c-75ab40cf6442"]'::jsonb)::text || '}')::jsonb
           end,
     last_modified_by_id   = (select id from users where username = 'beulah@apfodishauat'),
     manual_update_history = 'added age in high risk'
from public.individual  ind
where
      enc.organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1)
  and encounter_type_id = (select id from public.encounter_type et where name = 'ANC' and organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1) )
  and enc.individual_id	=  ind.id  ;
...
set observations  =
        case
           when (enc.observations ->> '68bc6e51-eb49-4816-b78b-2427bbab8d92')::numeric < 7   -- HB in encounter
                then enc.observations ||
                     ('{"9a7f284b-251d-459b-97d9-929ed280b3d3":' || (case when enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' is null then '[]' else enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' end || '["6e277f82-5822-4ac3-8a5a-d1ad21a79c00"]'::jsonb)::text || '}')::jsonb
           end,
     last_modified_by_id   = (select id from users where username = 'beulah@apfodishauat'),
     manual_update_history = 'added hb in high risk'
where
      enc.organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1)
  and encounter_type_id = (select id from public.encounter_type et where name = 'ANC' and organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1) );
...
set observations  =
        case
          when enc.observations ->> '96b167e1-2d98-40b9-af04-8e4f64f9999a' = '8ebbf088-f292-483e-9084-7de919ce67b7' -- Pregnancy geographically high risk in encounter
                then enc.observations ||
                     ('{"9a7f284b-251d-459b-97d9-929ed280b3d3":' || (case when enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' is null then '[]' else enc.observations->'9a7f284b-251d-459b-97d9-929ed280b3d3' end || '["3e21a6f4-6727-44b3-9b7e-148ce9ad77fd"]'::jsonb)::text || '}')::jsonb
           end,
     last_modified_by_id   = (select id from users where username = 'beulah@apfodishauat'),
     manual_update_history = 'added geographical in high risk'
where
      enc.organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1)
  and encounter_type_id = (select id from public.encounter_type et where name = 'ANC' and organisation_id = (select id from public.organisation o where name = 'APF Odisha UAT' limit 1) );
